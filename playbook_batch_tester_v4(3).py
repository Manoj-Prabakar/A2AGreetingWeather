...
def run_single_detect_intent(
    ...
    try:
        ...
        # Read directly from proto object — avoids MessageToDict camelCase issues
        qr = response.query_result

        # ── Detected intent — read directly from qr.match.intent proto ───────
        # Your debug output showed: match -> intent -> name (resource path)
        # We read display_name directly from the proto, which is the intent name
        detected_intent = ""
        try:
            detected_intent = qr.match.intent.display_name
        except Exception:
            pass

        # Fallback: extract from intent resource path last segment
        if not detected_intent:
            try:
                intent_name_resource = qr.match.intent.name
                detected_intent = intent_name_resource.split("/")[-1] if intent_name_resource else ""
            except Exception:
                pass
