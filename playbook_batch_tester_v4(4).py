...
def run_single_detect_intent(
    ...
    try:
        ...
        # ── Extract headintent from session parameters ─────────────────────────
        # The simulator stores the matched intent inside session parameters
        # under the key 'headintent'. This is the correct way to get the
        # detected intent for this playbook workstream.
        detected_intent = ""
        params_dict     = {}
        try:
            raw_params  = qr.parameters
            params_dict = dict(raw_params)
            # headintent value is the intent name set by the playbook/webhook
            head_intent_val = params_dict.get("headintent", "")
            if isinstance(head_intent_val, dict):
                # proto Struct wraps strings as {"stringValue": "..."} sometimes
                detected_intent = head_intent_val.get("stringValue", str(head_intent_val))
            else:
                detected_intent = str(head_intent_val).strip() if head_intent_val else ""
        except Exception:
            pass
