...
def run_single_detect_intent(
    ...
    WHY current_playbook WAS EMPTY in previous version:
    ────────────────────────────────────────────────────
    For playbook agents, dfcx_scrapi wraps the raw protobuf response and the
    current_playbook info does NOT live directly at qr.current_playbook as a
    plain string. Instead dfcx_scrapi provides a dedicated helper method:
        sessions_obj.collect_playbook_responses(response)
    which returns a dict with keys: current_playbook, response_text, parameters.

    We also fall back to inspecting the raw protobuf directly via MessageToDict
    so we can debug exactly what fields are being returned if needed.
    """
    ...
    try:
        ...
        # Equivalent to flow_display_name + page_display_name in flow workstream
        ...
        # ── Use collect_playbook_responses to parse playbook-specific fields ──
        # This is the correct dfcx_scrapi method for playbook agents.
        # It handles the protobuf parsing internally and returns clean fields.
        playbook_data = sessions_obj.collect_playbook_responses(response)
        # playbook_data keys: current_playbook, response_text, parameters

        current_playbook_name     = playbook_data.get("current_playbook", "")
        current_playbook_resource = playbook_data.get("current_playbook_resource", "")
        response_text             = playbook_data.get("response_text", "")
        params_dict               = playbook_data.get("parameters", {})

        # ── Fallback: if collect_playbook_responses returns empty, inspect raw ─
        # Convert full response to dict for debugging — printed only if empty
        if not current_playbook_name:
            try:
                raw_dict = MessageToDict(response._pb)
                qr_raw   = raw_dict.get("queryResult", {})

                # Try top-level currentPlaybook key
                current_playbook_resource = qr_raw.get("currentPlaybook", "")

                # Also check inside diagnosticInfo if present
                if not current_playbook_resource:
                    diag = qr_raw.get("diagnosticInfo", {})
                    current_playbook_resource = diag.get("currentPlaybook", "")

                # Extract display name from last segment of resource path
                ...
                # Parameters fallback
                if not params_dict:
                    params_dict = qr_raw.get("parameters", {})

                print(f"[DEBUG] Raw queryResult keys: {list(qr_raw.keys())}")
                print(f"[DEBUG] current_playbook resolved: '{current_playbook_name}'")

            except Exception as parse_err:
                print(f"[DEBUG] Raw parse error: {parse_err}")
