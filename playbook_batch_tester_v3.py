...
def run_single_detect_intent(
    ...
    """
    Call Sessions.detect_intent() for one utterance and return a parsed dict.

    playbook_resource_path: Full resource path of the initial playbook to route to.
        Format: projects/<proj>/locations/<loc>/agents/<agent>/playbooks/<playbook_id>
        This is passed as QueryParameters.current_playbook so Dialogflow CX starts
        the session at the specified playbook — equivalent to how flow workstream
        passed flow_display_name + page_display_name in the test_set DataFrame.

    Sessions.detect_intent() returns a QueryResult object (protobuf).
    For PLAYBOOK agents the fields we care about are:
        query_result.current_playbook   - which playbook handled this turn
        query_result.response_messages  - agent text responses
        query_result.parameters         - session parameters (entities)

    NOTE: There is NO detected_intent or confidence for playbook agents.
          The LLM routes the turn; there is no NLU confidence score.
    """
    session_id = str(uuid.uuid4())   # fresh session per utterance

    try:
        # Build QueryParameters with current_playbook set to force the
        # session to start at the specified playbook — this is the playbook
        # equivalent of flow_display_name + page_display_name in flow workstream.
        query_params = QueryParameters(
            current_playbook=playbook_resource_path
        )

        # Sessions.detect_intent signature:
        #   detect_intent(agent_id, session_id, text, language_code, query_params)
        ...
        # response is a DetectIntentResponse; the result lives in query_result
        qr = response.query_result

        # ── Which playbook handled this turn? ─────────────────────────────────
        # current_playbook is a resource path like:
        #   projects/.../agents/.../playbooks/<playbook_id>
        # We extract just the last segment as the "name".
        current_playbook_resource = getattr(qr, "current_playbook", "") or ""
        ...
        # ── Agent response text ───────────────────────────────────────────────
        response_texts = []
        for msg in getattr(qr, "response_messages", []):
            texts = getattr(msg.text, "text", [])
            response_texts.extend(texts)

        # ── Session parameters (entities extracted by the playbook) ───────────
        params_dict = {}
        raw_params = getattr(qr, "parameters", None)
        if raw_params:
            # Parameters come back as a google.protobuf.Struct
