...
def run_single_detect_intent(
    ...
    try:
        ...
        # ── Extract intent name from match.intent.name ──────────────────────
        # From debug output, the intent lives at:
        #   queryResult -> match -> intent -> name
        # which is a resource path like:
        #   projects/.../agents/.../intents/<intent_id>
        # We also get the intent displayName if available directly,
        # otherwise fall back to reading it from the match block.
        #
        # This is the playbook equivalent of detected_intent in flow workstream.
        intent_resource = (
            qr_raw
            .get("match", {})
            .get("intent", {})
        ...
        # Try displayName directly first
        ...
        # Fallback: extract last segment of resource path as intent ID
        if not current_playbook_name and intent_resource:
            current_playbook_name = intent_resource.split("/")[-1]

        current_playbook_resource = intent_resource
        ...
        return {
            "detected_intent":          current_playbook_name,
            "intent_resource":          current_playbook_resource,
...
def main():
    ...
    results = pd.DataFrame({
        ...
        "intent_resource":          [r["intent_resource"]            for r in raw_results],
    ...
    print(results[["utterance", "expected_playbook", "detected_intent",
